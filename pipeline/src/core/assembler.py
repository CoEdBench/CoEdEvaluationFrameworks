...
def _get_lang_tag(file_path: str) -> str:
    """Return Markdown code block language tag from file extension, empty string for unknown types."""
...
def _build_unified_diff(
    ...
    difflib.unified_diff does not support fromfilestart parameter,
    Use regex post-processing on @@ headers to add real line offset.

    from_lineno / to_lineno: 1-based start line of the snippet in the file.
    """
    ...
    # Fix line numbers in @@ -l,s +l,s @@ (difflib generates relative line numbers)
    ...
    for line in raw:
        ...
        if m:
            from_start = int(m.group(2)) + from_offset
            to_start   = int(m.group(5)) + to_offset
            from_count = m.group(3) or ""   # e.g. ",5" or "" (omitted for single line)
            to_count   = m.group(6) or ""
            tail       = m.group(7)
            line = f"@@ -{from_start}{from_count} +{to_start}{to_count}{tail}"
        result.append(line)
...
class PromptAssembler:
    ...
    # ──────────────────────────────────────────────────────────────
    # Stage 1: Identify impacted files and line numbers
    # ──────────────────────────────────────────────────────────────
    ...
    def assemble(
        ...
        Root Change diff uses actual file line numbers (old_start_line as offset).
        """
        root_hunk = parsed_item.root_hunk

        # Generate diff using actual line numbers
        root_diff = _build_unified_diff(
        ...
        user_prompt   = self._stage1_user_prompt(
...
```"""
    ...
    def assemble_fix(
        ...
        Args:
          ...
          current_version  - Code snippet extracted at line number ±10 (pure code, no line prefix)
          reasoning        - Stage 1 reasoning output (change pattern description)
          impacted_lines   - Stage 1 impacted line numbers, used to guide LLM
