...
class AgenticLoop(AgenticFlow):
    ...
    async def _run(self, message: str | None = None, node_id: int | None = None) -> tuple[Node, str | None]:
        ...
        while node_id or not (finish_reason := self.is_finished()):
            ...
            total_cost = usage.completion_cost
        ...
        final_usage = self.total_usage()
        logger.info(
            "Loop finished with %s iterations, cost=%s, prompt_tokens=%s, completion_tokens=%s, total_tokens=%s",
            len(self.root.get_all_nodes()),
            final_usage.completion_cost,
