...
MODEL_COST_ALIASES = {
    ...
    "openai/deepseek-reasoner": "deepseek/deepseek-reasoner",
}


def _normalize_model_for_cost(model: str) -> str:
    """Normalize model id to the keys used in MODEL_COSTS."""
    if model in MODEL_COSTS:
        return model

    alias = MODEL_COST_ALIASES.get(model)
    if alias:
        return alias

    if "/" in model:
        _, tail = model.split("/", 1)
        if tail in MODEL_COSTS:
            return tail
        alias = MODEL_COST_ALIASES.get(tail)
        if alias:
            return alias
...
class Usage(BaseModel):
    ...
    def calculate_cost(
        ...
        normalized_model = _normalize_model_for_cost(model)
        if normalized_model not in MODEL_COSTS:
            return 0.0

        rates = MODEL_COSTS[normalized_model]
    ...
    def get_total_prompt_tokens(self, model: str) -> int:
        ...
        if _normalize_model_for_cost(model) not in MODEL_COSTS:
    ...
    def update_from_response(self, completion_response: dict | BaseModel, model: str) -> "Usage":
        ...
        try:
            ...
            raw_cost = litellm_completion_cost(completion_response=completion_response, model=model)
            litellm_cost = float(raw_cost or 0.0)
        except Exception as e:
            logger.debug(f"litellm completion_cost failed for model={model}: {e}")

        if litellm_cost > 0:
            self.completion_cost = litellm_cost
            return self
        ...
        try:
            ...
            token_based_cost = float(prompt_cost or 0.0) + float(completion_cost or 0.0)
            if token_based_cost > 0:
                self.completion_cost = token_based_cost
                return self
        except Exception as e:
            logger.debug(f"litellm cost_per_token failed for model={model}: {e}")

        # Use local static table as last fallback.
