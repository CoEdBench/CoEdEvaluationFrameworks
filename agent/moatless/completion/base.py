...
try:
    # Newer LiteLLM versions expose RateLimitError under litellm.exceptions
    ...
    try:
        ...
        # Fallback for versions that only expose RouterRateLimitError
        from litellm.files.main import RouterRateLimitError as RateLimitError
...
class BaseCompletionModel(MoatlessComponent, ABC):
    ...
    def initialize(
        ...
        if self.model_base_url:
            ...
            self._completion_params["base_url"] = self.model_base_url
                ...
                try:
                    ...
                    try:
                        if not completion_response.choices:
                            logger.error(f"Completion response has no choices: {completion_response.model_dump_json(indent=2)}")
                            if invocation.current_attempt:
                        ...
                        if not message:
                            logger.error(f"Completion response has no message: {completion_response.model_dump_json(indent=2)}")
                            if invocation.current_attempt:
                                invocation.current_attempt.success = False
                                invocation.current_attempt.failure_reason = "Empty response"
                            raise CompletionRejectError(
                        ...
                        if not message.content and not message.tool_calls:
                            ...
                            has_reasoning = bool(
                                getattr(message, "reasoning_content", None) or getattr(message, "thinking_blocks", None)
                            )

                            # Kimi and similar "thinking-first" providers can return reasoning-only chunks
                            # that hit length before emitting a tool call. Treat this as retryable.
                            if finish_reason == "length" or has_reasoning:
                                raise CompletionRetryError(
                                    "Response had no content/tool calls (likely reasoning-only).",
                                    retry_message=ChatCompletionUserMessage(
                                        role="user",
                                        content=(
                                            "Your previous response did not include any tool call or content. "
                                            "Respond with exactly one valid tool call now. Keep thoughts concise."
                                        ),
                                    ),  # type: ignore
                                )
                        ...
                        message = completion_response.choices[0].message if completion_response.choices else None
                        should_append_assistant_msg = bool(
                            message
                            and (message.content or message.tool_calls)
                        )

                        if message and should_append_assistant_msg:
                            msg_dict = message.model_dump()
                            keep_reasoning_content = (
                                os.getenv("MOATLESS_INCLUDE_REASONING_CONTENT", "").strip().lower()
                                in {"1", "true", "yes", "on"}
                            )
