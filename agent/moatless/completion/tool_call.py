...
class ToolCallCompletionModel(BaseCompletionModel):
    ...
    def _get_completion_params(self, schema: list[type[ResponseSchema]]) -> dict[str, Union[str, dict, list]]:
        ...
        # Default behavior: use auto.
        # Escape hatch:
        # - MOATLESS_TOOL_CHOICE=auto forces auto for all models.
        # - MOATLESS_TOOL_CHOICE=required forces required for all models.
        env_tool_choice = os.getenv("MOATLESS_TOOL_CHOICE", "").strip().lower()
        if env_tool_choice in {"auto", "required"}:
            tool_choice = env_tool_choice
            logger.info("Using tool_choice override=%s for model=%s", tool_choice, self.model)
        else:
            if env_tool_choice:
                logger.warning(
                    "Invalid MOATLESS_TOOL_CHOICE=%r, fallback to auto (allowed: auto|required)",
                    env_tool_choice,
                )
            tool_choice = "auto"
        ...
        return {
            ...
            # Keep tool-calling configured; default to auto tool choice unless explicitly overridden.
