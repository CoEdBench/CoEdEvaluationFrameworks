...
class MessageHistoryGenerator(BaseMemory):
    ...
    async def _generate_token_limited_messages(
        ...
        # ── Fix orphaned tool messages and dangling tool_calls ─────────────────
        # Token-limited selection can produce two API-rejected states:
        #   1. A tool response is kept but the assistant(tool_calls) before it
        #      was dropped → remove orphaned tool messages.
        #   2. An assistant(tool_calls) is kept but one or more tool responses
        #      that follow it were dropped → strip tool_calls from the assistant
        #      (preserving any text content).
        #
        # Pass 1: for each assistant(tool_calls), check if ALL its declared ids
        #         are fulfilled by subsequent tool messages.
        declared_at: dict[int, set[str]] = {}   # orig index → declared tool ids
        fulfilled_at: dict[int, set[str]] = {}  # orig index → fulfilled ids
        ...
        for i, msg in enumerate(selected_messages):
            if isinstance(msg, dict) and msg.get("role") == "assistant":
                ...
                if tcs:
                    tids = {tc["id"] for tc in tcs if isinstance(tc, dict) and tc.get("id")}
                    if tids:
                        declared_at[i] = tids
                        fulfilled_at[i] = set()
                        open_idx, open_ids = i, tids.copy()
            elif isinstance(msg, dict) and msg.get("role") == "tool" and open_idx is not None:
                tcid = msg.get("tool_call_id")
                if tcid and tcid in open_ids:
                    open_ids.discard(tcid)
                    fulfilled_at[open_idx].add(tcid)
                    if not open_ids:
                        open_idx = None

        # Pass 2: build filtered list
        # - assistants whose ids were not ALL fulfilled → strip tool_calls
        # - tool messages whose id was never declared → drop
        valid_ids: set[str] = set()
        filtered: list[AllMessageValues] = []
        for i, msg in enumerate(selected_messages):
            if isinstance(msg, dict) and msg.get("role") == "assistant":
                if i in declared_at:
                    if fulfilled_at[i] == declared_at[i]:
                        valid_ids = declared_at[i].copy()
                    else:
                        msg = dict(msg)
    ...
    async def _process_assistant_message(
        ...
        if node.thoughts and node.thoughts.text:
            # Reasoning models (deepseek-reasoner, deepseek-v4-pro, kimi with thinking, etc.)
            # require reasoning_content to be present on assistant messages in multi-turn history.
            assistant_message["reasoning_content"] = node.thoughts.text
        elif os.getenv("MOATLESS_INCLUDE_REASONING_CONTENT", "").strip().lower() in {"1", "true", "yes", "on"}:
