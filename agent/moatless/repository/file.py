...
class FileRepository(Repository):
    ...
    def get_full_path(self, file_path: str) -> str:
        ...
        repo_dir_normalized = self.repo_dir.replace("\\", "/")

        # Strip leading slash if present
        file_path = file_path.lstrip("/")

        # If file_path starts with repo_dir, make it relative
        if file_path.startswith(repo_dir_normalized):
            file_path = file_path.replace(repo_dir_normalized, "").lstrip("/")
        ...
        # 使用 pathlib 进行安全的跨平台路径拼接，并强制输出为正斜杠格式
        return Path(self.repo_path).joinpath(file_path).as_posix()
    ...
    def get_relative_path(self, file_path: str) -> str:
        ...
        # 确保 repo_path 也是正斜杠格式，以便进行字符串替换
        repo_path_normalized = self.repo_path.replace("\\", "/")

        rel_path = full_path.replace(repo_path_normalized, "")
    ...
    def get_file(self, file_path: str):
        ...
        if file_path.startswith(repo_dir_normalized):
            file_path = file_path.replace(repo_dir_normalized, "")
            if file_path.startswith("/"):
                file_path = file_path[1:]

        full_file_path = self.get_full_path(file_path)

        # Python 的 os.path.exists 和 os.path.isfile 在 Windows 上完美支持正斜杠
