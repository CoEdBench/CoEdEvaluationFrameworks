...
class ActionAgent(MoatlessComponent):
    ...
    async def _generate_actions(self, node: Node):
        ...
        # Always rebind repository via property setter to ensure all existing ContextFile
        # instances carry the current repo reference (some legacy states can keep _repo
        # on FileContext while child ContextFile objects are not rebound).
        if node.file_context and self._workspace:
            node.file_context.repository = self._workspace.repository
            node.file_context._runtime = self._workspace.runtime
